...
def viterbi(acts, targets):
    ...
    D = np.zeros((num_targets_ext, num_frames))
    F = np.zeros((num_targets_ext, num_frames))
    D[0, 0] = acts[0, 0]
    for t in range(1, num_frames):
        D[0, t] = cumsum_acts[t, 0]
    ...
    for t_prev in range(1, num_frames):
        tmp = D[num_targets_ext-2, t_prev] + cumsum_acts[num_frames-1, 0] - cumsum_acts[t_prev, 0]
        if tmp > max_tmp:
            max_tmp = tmp
            t_prev_best = t_prev
